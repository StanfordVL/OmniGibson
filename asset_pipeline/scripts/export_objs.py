...
def process_link(G, link_node, base_link_center, canonical_orientation, obj_name, obj_dir, tree_root):
    ...
    if len(in_edges) == 1:
        ...
        # Load the meshes.
        parent_canonical_mesh = transform_mesh(G.nodes[parent_node]["lower_mesh"], base_link_center, canonical_orientation)
        lower_canonical_mesh = transform_mesh(G.nodes[child_node]["lower_mesh"], base_link_center, canonical_orientation)
        ...
        if joint_type == "R":
            # Revolute joint
            num_v = lower_canonical_mesh.vertices.shape[0]
