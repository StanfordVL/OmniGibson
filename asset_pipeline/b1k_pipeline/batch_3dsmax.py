...
def main():
    ...
    assert scene_file.exists() and scene_file.suffix == ".max", f"Can't find {scene_file}."

    script_file = (b1k_pipeline.utils.PIPELINE_ROOT / sys.argv[2]).absolute()
    assert script_file.exists() and script_file.suffix == ".py", f"Can't find {script_file}."
    ...
    if USE_RPC:
        ...
        subprocess.run([THREEDSMAX_PATH, str(script_file), "-sceneFile", str(scene_file)] + sum(max_args))
