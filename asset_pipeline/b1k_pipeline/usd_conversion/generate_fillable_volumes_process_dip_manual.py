...
def process_object(cat, mdl, out_path):
    ...
    points = points[np.all(points <= (aabb_max + np.array([0, 0, water.particle_radius + 1])) , axis=1)]
...
def main():
    ...
    batch = sys.argv[2:]
    for item in batch:
        obj_category, obj_model = item.split("-")
        obj_dir = pathlib.Path(gm.DATASET_PATH) / "objects" / obj_category / obj_model
