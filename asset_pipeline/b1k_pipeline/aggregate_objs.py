...
def main():
    ...
    try:
        ...
        for obj, provider in object_inventory["providers"].items():
            ...
            src_path = b1k_pipeline.utils.PIPELINE_ROOT / "cad" / provider / "artifacts" / "objects" / obj
            assert src_path.exists(), f"Could not resolve path {src_path} for object {obj} provided by {provider}."

            dst_path = objects_root_dir / obj
    ...
    filename = metadata_output_dir / OUTPUT_FILENAME
    with open(filename, "w") as f:
        json.dump({"success": success, "error_msg": error_msg}, f, indent=4)

    if success:
        with open(metadata_output_dir / SUCCESS_FILENAME, "w") as f:
            pass
