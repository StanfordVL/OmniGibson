...
class SanityCheck:
  ...
  def is_valid_name(self, row):
    is_valid_name = not pd.isna(row.object_name) and b1k_pipeline.utils.parse_name(row.object_name) is not None
  ...
  def run(self):
    ...
    col_to_name = {v-1: "name_" + k for k, v in b1k_pipeline.utils.NAME_PATTERN.groupindex.items()}
    applied_df = df.apply(lambda row: list(b1k_pipeline.utils.parse_name(row.object_name).groups()), axis='columns', result_type='expand')
