...
if False:
    ...
    positions = [np.mean(rt.polyop.getVerts(obj, list(range(1, rt.polyop.getNumVerts(obj) + 1))), axis=0) for obj in keys]
...
if True:
    ...
    for i, key in enumerate(keys):
        if not rt.isValidNode(key):
            continue
        candidates = [keys[a] for a in np.nonzero(above_threshold[i])[0] if rt.isValidNode(keys[a])]
        if len(candidates) > 1:
            pair = tuple({x.baseObject for x in candidates})
            assert len(pair) == 2, "Whoa! " + str(candidates) + " has weird pair " + str(pair)
            if len(pair) == 2:
