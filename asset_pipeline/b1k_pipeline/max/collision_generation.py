...
VHACD_EXECUTABLE = r"D:\ig_pipeline\b1k_pipeline\vhacd.exe"
...
def run_vhacd(input_mesh, hull_count):
    with tempfile.TemporaryDirectory() as td:
        ...
        vhacd_cmd = [str(VHACD_EXECUTABLE), in_path, "-r", 1000000, "-d", 20, "-f", "flood", "-e", 10, "-p", True, "-l", 2, "-v", 60, "-h", hull_count]
        try:
...
def generate_collision_mesh(obj):
    ...
    for method_name, method in USE_METHODS.items():
        for hull_count in HULL_COUNTS:
            ...
            print(f"Generated {method_name} w/ max hull count {hull_count}, actual hull count {len(result)}, name {collision_obj.name}")
