...
2. All output files are written to `<gm.EXTERNAL_DATASET_PATH>/objects/robot/<name>`. Please copy / move this directory to `<gm.ASSET_PATH>/objects/<name>` so it can be imported into **OmniGibson**.
3. That's it!
...
- The importing procedure can be summarized as follows:
  ...
  9. Removes all friction from all joints
  10. Generates holonomic base joints (as specified by the source config)
  11. Generates configuration files needed for CuRobo motion planning (as specified by the source config)
