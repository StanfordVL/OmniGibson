...
def waypoint_to_velocity(waypoint: np.ndarray, max_v: float, max_w: float, dt: float) -> np.ndarray:
    ...
    if (abs(dx) < EPS) and (abs(dy) < EPS):
        ...
        # 단순 매핑: dx/dt -> v, arctan2(dy, dx)/dt -> w
        v = dx / dt
...
def sample_diffusion_action(
    ...
    """
    obs_images : 최근 관측 이미지 목록 (PIL)
    model_params: num_diffusion_iters, context_size, image_size, len_traj_pred 등
    noise_scheduler: 확산 스케줄러
...
def main(random_selection=False, headless=False, short_exec=False):
    ...
    for ep_i in range(max_iterations):
        ...
        for step_i in range(steps_per_ep):
            ...
            # 4) Diffusion 기반 (dx, dy) -> (v, w)
            if len(context_queue) > context_size:
                # (a) NoMaD 모델로부터 (dx, dy) 추론
                ...
                # (b) (dx, dy) --> (v, w) 변환
                action_vw = waypoint_to_velocity(waypoint_dxdy, MAX_V, MAX_W, DT)
                action = action_vw  # shape (2,)
