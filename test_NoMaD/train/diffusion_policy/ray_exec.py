...
def worker_fn(command_args, data_src=None, unbuffer_python=False, use_shell=False):
    ...
    process = subprocess.Popen(args=command_args, env=process_env, preexec_fn=preexec_function, shell=use_shell)
...
def main(ray_address, num_cpus, num_gpus, max_retries, data_src, unbuffer_python, command_args):
    # expand path
    if data_src is not None:
