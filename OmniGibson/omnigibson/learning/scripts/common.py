...
def get_credentials(credentials_path: str) -> Tuple[gspread.Client, str]:
    ...
    response = requests.post(LIGHTWHEEL_LOGIN_URL, json={
        "username": lightwheel_api_credentials["username"],
        "password": lightwheel_api_credentials["password"]
    })
...
def download_and_extract_data(
    ...
    assert os.path.exists(f"{data_dir}/raw/{base_name}/{task_name}.hdf5"), f"File not found: {data_dir}/raw/{base_name}/{task_name}.hdf5"
    ...
    with open(f"{data_dir}/raw/{base_name}/running_args.json", "r") as f:
        ...
        assert running_args["instance_id"] == instance_id, f"Instance ID mismatch: {running_args['instance_id']} != {instance_id}"
    os.rename(
