...
class OpenPi():
    ...
    def act_receeding_temporal(self, input_obs):
        ...
        if self.step_counter % self.replan_interval == 0:
            ...
            batch = {
                "observation/egocentric_camera": resize_with_pad(nbatch["observation"][0, 0].numpy(), RESIZE_SIZE, RESIZE_SIZE),
                "observation/wrist_image_left": resize_with_pad(nbatch["observation"][0, 1].numpy(), RESIZE_SIZE, RESIZE_SIZE),
                "observation/wrist_image_right": resize_with_pad(nbatch["observation"][0, 2].numpy(), RESIZE_SIZE, RESIZE_SIZE),
    ...
    def process_obs(self, obs):
        ...
        """
        observation = obs["obs"]
        prop_state = generate_prop_state(observation["robot_r1::proprio"][None])
        img_obs = torch.stack([
            observation["robot_r1::robot_r1:zed_link:Camera:0::rgb"][None,...,:3],
            observation["robot_r1::robot_r1:left_realsense_link:Camera:0::rgb"][None,...,:3],
            observation["robot_r1::robot_r1:right_realsense_link:Camera:0::rgb"][None,...,:3],
        ], axis=1)
        new_obs = {
