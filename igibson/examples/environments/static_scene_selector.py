...
def main(random_selection=False, headless=False, short_exec=False):
    ...
    config_filename = os.path.join(ig.example_config_path, "turtlebot_static_nav.yaml")
    config = yaml.load(open(config_filename, "r"), Loader=yaml.FullLoader)

    # Choose the scene to load, modify the config appropriately, and create the environment
    g_scenes = get_available_g_scenes()
    scene_model = choose_from_options(options=g_scenes, name="scene model", random_selection=random_selection)
