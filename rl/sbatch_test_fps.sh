...
${MOUNT_KWARGS} \
${CONTAINER_NAME} \
micromamba run -n omnigibson /bin/bash --login -c "source /isaac-sim/setup_conda_env.sh && pip install git+https://github.com/StanfordVL/bddl@develop gymnasium protobuf==3.20.2 grpcio grpcio-tools stable_baselines3 wandb tensorboard moviepy && cd /omnigibson-src/workspace && python -u /omnigibson-src/rl/test_multiple_envs.py"
