...
def main(dirs):
    ...
    action_space = gym.spaces.Box(
        low=np.array([-1.0, -1.0, -1.0, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -1.0]),
        high=np.array([1.0, 1.0, 1.0, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, 1.0]),
        shape=(12,),
        dtype=np.float32,
    )
    observation_space = gym.spaces.Dict(
    ...
    config = (
        ...
        .offline_data(
            ...
            # )
            input_=lambda ioctx: ShuffledInput(CustomReader(dirs, ioctx))
        )
