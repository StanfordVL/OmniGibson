...
${MOUNT_KWARGS} \
${CONTAINER_NAME} \
micromamba run -n omnigibson /bin/bash --login -c "source /isaac-sim/setup_conda_env.sh && pip install git+https://github.com/StanfordVL/bddl@develop gymnasium protobuf==3.20.2 && cd /omnigibson-src/rl && python generate_random_poses_2.py $1 ${SLURM_ARRAY_TASK_ID}.json"
