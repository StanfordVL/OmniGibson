...
enroot start \
    ...
    ${MOUNT_KWARGS} \
    ${CONTAINER_NAME} \
    micromamba run -n omnigibson /bin/bash --login -c "source /isaac-sim/setup_conda_env.sh && pip install gymnasium grpcio grpcio-tools stable_baselines3 wandb tensorboard moviepy && cd /omnigibson-src/workspace && WANDB_API_KEY=$3 python -u /omnigibson-src/rl/service/sweep/omni_grpc_learner_sweep.py --n_envs $1 --port $2 --sweep_id $4"
