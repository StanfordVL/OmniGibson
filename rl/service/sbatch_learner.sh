...
enroot start \
    ...
    ${MOUNT_KWARGS} \
    ${CONTAINER_NAME} \
    micromamba run -n omnigibson /bin/bash --login -c "source /isaac-sim/setup_conda_env.sh && pip install gymnasium grpcio grpcio-tools stable_baselines3 wandb moviepy && cd /omnigibson-src/rl/service && python omni_grpc_learner.py --n_envs $1 --port $2"
