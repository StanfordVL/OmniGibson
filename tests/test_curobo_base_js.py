...
motion_kwargs = dict(
                ...
                ik_opt_iters=100,
                optimize_dt=True,
                num_trajopt_seeds=16, # originally 4
                num_graph_seeds=16, # originally 4
                interpolation_dt=0.03,
                collision_activation_distance=0.05,
                self_collision_check=True, # originally True
...
retract_cfg = motion_generator.mg["base"].get_retract_config()
start_state = lazy.curobo.types.state.JointState.from_position(retract_cfg.view(1, -1))

goal_cfg = th.tensor([ 1.447, 0.477, 2.91], device="cuda")
goal_state = lazy.curobo.types.state.JointState.from_position(goal_cfg.view(1, -1))

motion_generator.update_obstacles()

result = motion_generator.mg["base"].plan_single_js(
            start_state,
            goal_state,
...
# q_traj = result.get_interpolated_plan().position
traj_path = result.get_interpolated_plan()
q_traj = motion_generator.path_to_joint_trajectory(traj_path, get_full_js=True, emb_sel=embodiment_selection).cpu()
