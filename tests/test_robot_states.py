...
def camera_pose_test(flatcache):
    ...
    assert np.allclose(sensor_world_pos, sensor_world_pos_gt, atol=1e-4)
    assert np.allclose(sensor_world_ori, sensor_world_ori_gt, atol=1e-4)
    ...
    assert np.allclose(old_camera_local_pose[0], new_camera_local_pose[0], atol=1e-4)
    assert np.allclose(new_camera_world_pose[0], expected_camera_world_pos, atol=1e-4)
    assert np.allclose(new_camera_world_pose[1], expected_camera_world_ori, atol=1e-4)
    ...
    assert np.allclose(new_camera_world_pose[0], expected_new_camera_world_pos, atol=1e-4)
    assert np.allclose(new_camera_world_pose[1], expected_new_camera_world_ori, atol=1e-4)
    assert np.allclose(robot.get_position(), [100, 100, 100], atol=1e-4)
    ...
    new_camera_local_pose = vision_sensor.get_local_pose()
    assert not np.allclose(old_camera_local_pose[0], new_camera_local_pose[0], atol=1e-4)
    assert not np.allclose(old_camera_local_pose[1], new_camera_local_pose[1], atol=1e-4)
    assert np.allclose(robot.get_position(), [150, 150, 100], atol=1e-4)
