...
def main():
    ...
    cfg["task"]["activity_name"] = "test_tiago_giftbox"
    ...
    # giftbox cup
    # Import task relevant objects
    gift_box = DatasetObject(name="gift_box", category="gift_box", model="mfalrc", scale=th.tensor([1.0, 1.0, 1.0]))
    objs_to_add = [gift_box]
    og.sim.batch_add_objects(objs_to_add, [env.scene] * len(objs_to_add))

    # Position them in the scene
    gift_box.set_position_orientation(gift_box_1_position, T.euler2quat(th.tensor([0, 0, -np.pi / 2.0])))
