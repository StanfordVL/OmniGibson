...
def main():
    ...
    cfg["task"]["activity_name"] = "test_tiago_notebook"
    ...
    # Assign to object scope
    env.task.object_scope["notebook.n.01_1"] = BDDLEntity(bddl_inst="notebook.n.01_1", entity=notebook)

    # # paper cup
    # # Import task relevant objects
    # crock_pot = DatasetObject(name="crock_pot", category="crock_pot", model="lspxjq", scale=th.tensor([.8, .8, .8]))
    # objs_to_add = [crock_pot]
    # og.sim.batch_add_objects(objs_to_add, [env.scene] * len(objs_to_add))

    # # Position them in the scene
    # crock_pot.set_position_orientation(pot_1_position, T.euler2quat(th.tensor([0, 0, -np.pi / 2.0])))
