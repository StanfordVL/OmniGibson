...
def main():
    ...
    for room in segmap.room_ins_name_to_ins_id:
        ...
        min_coords = np.min(room_pixels_in_world, axis=0)
        max_coords = np.max(room_pixels_in_world, axis=0)

        # Compute room dimensions (length and width)
        room_length = max_coords[0] - min_coords[0]
        room_width = max_coords[1] - min_coords[1]

        # Convert FOV from degrees to radians
        #fov_horizontal_rad = np.radians(fov)
        #fov_vertical_rad = np.radians(fov)

        # Calculate the distances for both horizontal and vertical FOVs
        distance_for_horizontal_fov = room_width / 2 / np.tan(fov / 2)
        distance_for_vertical_fov = room_length / 2 / np.tan(fov / 2)

        # The required height is the maximum of the two distances
        camera_height = max(distance_for_horizontal_fov, distance_for_vertical_fov)
        print(camera_height)
