...
def main(random_selection=False, headless=False, short_exec=False):
    ...
    apples[0].set_position_orientation(
        position=stove.states[object_states.HeatSourceOrSink].link.get_position() + th.tensor([0.11, 0, 0.1])
    )
    ...
    apples[1].set_position_orientation(
        position=stove.states[object_states.HeatSourceOrSink].link.get_position() + th.tensor([0.32, 0, 0.1])
    )
