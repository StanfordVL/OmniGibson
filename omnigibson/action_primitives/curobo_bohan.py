...
def create_world_mesh_collision(tensor_args, obb_cache_size=10, mesh_cache_size=2048, max_distance=0.05):
    ...
    world_cfg = lazy.curobo.geom.sdf.world.WorldCollisionConfig.load_from_dict(
        dict(cache={"obb": obb_cache_size, "mesh": mesh_cache_size}, n_envs=1,
             checker_type=lazy.curobo.geom.sdf.world.CollisionCheckerType.MESH, max_distance=max_distance, ),
...
def get_obstacles(reference_prim_path=None, only_paths=None, ignore_paths=None, only_substring=None,
    ...
    return usd_help.get_obstacles_from_stage(reference_prim_path=reference_prim_path, only_paths=only_paths,
                                             ignore_paths=ignore_paths, only_substring=only_substring,
                                             ignore_substring=ignore_substring, ).get_collision_check_world()  # WorldConfig


def get_obstacles_sphere_representation(obstacles, tensor_args, n_spheres=20, sphere_radius=0.001, ):
    """
    ...
    for obs, n_sph, sph_radius in zip(obstacles, n_spheres, sphere_radius):
        sph = obs.get_bounding_spheres(n_sph, sph_radius,
                                       pre_transform_pose=lazy.curobo.types.math.Pose(position=th.zeros(3),
                                                                                      quaternion=th.tensor(
                                                                                          [1.0, 0, 0, 0])).to(
                                           tensor_args), tensor_args=tensor_args,
...
class CuRoboMotionGenerator:
    ...
    def __init__(self, robot, robot_cfg_path=None, robot_usd_path=None, device="cuda:0", motion_cfg_kwargs=None,
                 batch_size=2, use_cuda_graph=True, debug=False, use_default_embodiment_only=False, ):
        """
        Args:
        ...
        world_coll_checker = create_world_mesh_collision(self._tensor_args, obb_cache_size=10, mesh_cache_size=2048,
                                                         max_distance=0.05)
        ...
        for emb_sel, robot_cfg_path in robot_cfg_path_dict.items():
            try:
                content_path = lazy.curobo.types.file_path.ContentPath(robot_config_absolute_path=robot_cfg_path,
                                                                       robot_usd_absolute_path=robot_usd_path)
                ...
                motion_kwargs = dict(trajopt_tsteps=32,
                                     collision_checker_type=lazy.curobo.geom.sdf.world.CollisionCheckerType.MESH,
                                     use_cuda_graph=use_cuda_graph, num_ik_seeds=128, num_batch_ik_seeds=128,
                                     num_batch_trajopt_seeds=1, num_trajopt_noisy_seeds=1, ik_opt_iters=100,
                                     optimize_dt=True, num_trajopt_seeds=4, num_graph_seeds=4, interpolation_dt=0.03,
                                     collision_activation_distance=0.025, self_collision_check=True,
                                     maximum_trajectory_dt=None, fixed_iters_trajopt=True, finetune_trajopt_iters=100,
                ...
                motion_gen_config = lazy.curobo.wrap.reacher.motion_gen.MotionGenConfig.load_from_robot_config(
                    robot_cfg=robot_cfg_obj, world_model=None, world_coll_checker=world_coll_checker,
                    tensor_args=self._tensor_args, store_trajopt_debug=self.debug, **motion_kwargs, )
    ...
    def save_visualization(self, q, file_path, emb_sel=CuroboEmbodimentSelection.DEFAULT):
        cu_js = lazy.curobo.types.state.JointState(position=self.tensor_args.to_device(q),
                                                   joint_names=self.robot_joint_names, ).get_ordered_joint_state(
            self.mg[emb_sel].kinematics.joint_names)
    ...
    def update_obstacles(self, ignore_paths=None, emb_sel=CuroboEmbodimentSelection.DEFAULT):
        ...
        obstacles = self._usd_help.get_obstacles_from_stage(reference_prim_path=self.robot.root_link.prim_path,
                                                            ignore_substring=[self.robot.prim_path,
                                                                              # Don't include robot paths
                                                                              "/curobo",  # Don't include curobo prim
                                                                              "visual",  # Don't include any visuals
                                                                              *METALINK_PREFIXES,
                                                                              # Don't include any metalinks
                                                                              *ignore_scenes,
                                                                              # Don't include any scenes the robot is not in
                                                                              *ignore_visual_only,
                                                                              # Don't include any visual-only objects
                                                                              *ignore_paths,
                                                                              # Don't include any additional specified paths
                                                                              ], ).get_collision_check_world()
        self.mg[emb_sel].update_world(obstacles)
        print("Synced CuRobo world from stage.")

    def check_collisions(self, q, check_self_collision=True, emb_sel=CuroboEmbodimentSelection.DEFAULT, ):
        """
    ...
    def update_locked_joints(self, cu_joint_state, emb_sel):
        ...
        if kc.lock_jointstate is not None:
            ...
            for joint_name in kc.lock_jointstate.joint_names:
                ...
                relative_pose = T.pose2mat(T.relative_pose_transform(*child_link.get_position_orientation(),
                                                                     *parent_link.get_position_orientation()))
                link_idx = kc.link_name_to_idx_map[child_link_name]
                kc.fixed_transforms[link_idx] = relative_pose

    def compute_trajectories(self, target_pos, target_quat, is_local=False, max_attempts=5, timeout=2.0,
                             ik_fail_return=5, enable_finetune_trajopt=True, finetune_attempts=1,
                             return_full_result=False, success_ratio=None, attached_obj=None,
        ...
        try:
            relevant_joint_positions_normalized = (lazy.curobo.types.state.JointState(
                position=self.tensor_args.to_device(self.robot.get_joint_positions(normalized=True)),
                joint_names=self.robot_joint_names, ).get_ordered_joint_state(
                self.mg[emb_sel].kinematics.joint_names).position)
            ...
            plan_cfg = lazy.curobo.wrap.reacher.motion_gen.MotionGenPlanConfig(enable_graph=False,
            ...
            cu_joint_state = lazy.curobo.types.state.JointState(position=self._tensor_args.to_device(q_pos),
            ...
            if attached_obj is not None:
                ...
                self.mg[emb_sel].attach_objects_to_robot(joint_state=cu_js_batch, object_names=obj_paths, )
            ...
            for i in range(n_batches):
                ...
                for link_name in target_pos.keys():
                    ...
                    # Create IK goal
                    ik_goal_batch = lazy.curobo.types.math.Pose(position=batch_target_pos,
                ...
                result = self.mg[emb_sel].plan_batch(cu_js_batch, main_ik_goal_batch, plan_cfg,
    ...
    def convert_q_to_eef_traj(self, traj, return_axisangle=False, emb_sel=CuroboEmbodimentSelection.DEFAULT):
        ...
        orientations = self._tensor_args.to_device(
            th.zeros((n, 4)))  # This will be quat initially but we may convert to aa representation
