...
from omnigibson.action_primitives.curobo_vectorized import CuRoboEmbodimentSelection, CuRoboMotionGenerator
...
m.MAX_STEPS_FOR_JOINT_MOTION = 2 # originally 10
...
class StarterSemanticActionPrimitives(BaseActionPrimitiveSet):
    def __init__(
        ...
        super().__init__(vec_env, robot)
        self.num_envs = len(vec_env.envs)
        ...
        for env_idx in range(len(vec_env.envs)):
            control_dict = self.robot[env_idx].get_control_dict()
            self._arm_targets = {}
            self._reset_eef_pose = {}
            if isinstance(self.robot[env_idx], ManipulationRobot):
                for arm_name in self.robot[env_idx].arm_names:
                    eef = f"eef_{arm_name}"
                    arm = f"arm_{arm_name}"
                    arm_ctrl = self.robot[env_idx].controllers[arm]
                    if isinstance(arm_ctrl, InverseKinematicsController):
                        ...
                        self._arm_targets[f"{arm}_env_{env_idx}"] = (pos_relative, quat_relative_axis_angle)
                    else:
                        arm_target = cb.to_torch(control_dict["joint_position"])[arm_ctrl.dof_idx]
                        self._arm_targets[f"{arm}_env_{env_idx}"] = arm_target

                    self._reset_eef_pose[f"{arm_name}_env_{env_idx}"] = self.robot[env_idx].get_relative_eef_pose(arm_name)
    ...
    def arm(self, env_idx=0):
        assert isinstance(self.robot[env_idx], ManipulationRobot), "Cannot use arm for non-manipulation robot"
        return self.robot[env_idx].default_arm
    ...
    def _get_obj_in_hand(self, env_idx=0):
        """
        ...
        """
        obj_in_hand = self.robot[env_idx]._ag_obj_in_hand[self.arm]  # TODO(MP): Expose this interface.
        return obj_in_hand
    ...
    def _open_or_close(self, obj, should_open):
        ...
        self._tracking_object = self.robot[env_idx]
        ...
        for _ in range(m.MAX_ATTEMPTS_FOR_OPEN_CLOSE):
            try:
                ...
                if should_open:
                    grasp_data = get_grasp_position_for_open(self.robot[env_idx], obj, should_open, None)
                else:
                    grasp_data = get_grasp_position_for_open(self.robot[env_idx], obj, should_open, None, num_waypoints=3)
                ...
                yield from self._move_hand_linearly_cartesian(
                    self.robot[env_idx].eef_links[self.arm].get_position_orientation(), ignore_failure=True, stop_if_stuck=True
                )
    ...
    def _move_base_backward(self, steps=5, speed=0.2):
        ...
        for _ in range(steps):
            ...
            action[self.robot[env_idx].controller_action_idx["gripper_{}".format(self.arm)]] = 1.0
            action[self.robot[env_idx].base_control_idx[0]] = -speed
    ...
    def _move_hand_backward(self, steps=5, speed=0.2):
        ...
        for _ in range(steps):
            action = self._empty_action()
            action[self.robot[env_idx].controller_action_idx["gripper_{}".format(self.arm)]] = 1.0
            action[self.robot[env_idx].controller_action_idx["arm_{}".format(self.arm)][0]] = -speed
            yield self._postprocess_action(action)
    ...
    def _move_hand_upward(self, steps=5, speed=0.1):
        ...
        for _ in range(steps):
            action = self._empty_action()
            action[self.robot[env_idx].controller_action_idx["gripper_{}".format(self.arm)]] = 1.0
            action[self.robot[env_idx].controller_action_idx["arm_{}".format(self.arm)][2]] = speed
            yield self._postprocess_action(action)

    def _sample_grasp_pose(self, obj, env_idx=0):
        """
        ...
        avg_finger_offset = th.mean(
            th.tensor([length for length in self.robot[env_idx].eef_to_fingertip_lengths[self.arm].values()])
        )
    ...
    def _grasp(self, obj):
        ...
        if self.robot[env_idx].grasping_mode not in ["sticky", "assisted"]:
            raise ActionPrimitiveError(
                ActionPrimitiveError.Reason.PRE_CONDITION_ERROR,
                "Grasping mode not supported",
                {"grasping mode": self.robot[env_idx].grasping_mode},
            )
        ...
        if self.robot[env_idx].grasping_mode == "sticky":
        ...
        elif self.robot[env_idx].grasping_mode == "assisted":
    ...
    def _toggle(self, obj, value):
        ...
        hand_orientation = self.robot[env_idx].eef_links[self.arm].get_position_orientation()[1]
    ...
    def _target_in_reach_of_robot(self, target_pose, env_idx=0, initial_joint_pos=None, skip_obstacle_update=False):
        """
    ...
    def _manipulation_control_idx(self, env_idx=0):
        """The appropriate manipulation control idx for the current settings."""
        return th.cat([self.robot[env_idx].trunk_control_idx, self.robot[env_idx].arm_control_idx[self.arm]])

    def _ik_solver_cartesian_to_joint_space(self, target_pose, env_idx=0, initial_joint_pos=None, skip_obstacle_update=False):
        """
        ...
        for arm, pose in target_pose.items():
            target_pos[self.robot[env_idx].eef_link_names[arm]] = pose[0]
            target_quat[self.robot[env_idx].eef_link_names[arm]] = pose[1]
        ...
        # # since we have vectorized env and in this case we only pass 1 env
        ...
        joint_state = joint_states[0][success_idx[0]]
        # breakpoint()
    ...
    def _move_hand(
        ...
        if not lock_auxiliary_arm:
            target_pos = {
                self.robot[env_idx].eef_link_names[self.arm]: target_pose[0],
            }
            target_quat = {
                self.robot[env_idx].eef_link_names[self.arm]: target_pose[1],
            }
        else:
            left_hand_pos, left_hand_quat = target_pose if self.arm == "left" else self.robot[env_idx].get_eef_pose(arm="left")
            right_hand_pos, right_hand_quat = (
                target_pose if self.arm == "right" else self.robot[env_idx].get_eef_pose(arm="right")
            )
            target_pos = {
                self.robot[env_idx].eef_link_names["left"]: left_hand_pos,
                self.robot[env_idx].eef_link_names["right"]: right_hand_pos,
            }
            target_quat = {
                self.robot[env_idx].eef_link_names["left"]: left_hand_quat,
                self.robot[env_idx].eef_link_names["right"]: right_hand_quat,
            }
    ...
    def _plan_joint_motion(
        ...
        obj_in_hand = self._get_obj_in_hand()
        attached_obj = {self.robot[0].eef_link_names[self.arm]: obj_in_hand.root_link} if obj_in_hand is not None else None
    ...
    def _execute_motion_plan(
        ...
        for i in range(len(q_trajs[0])):
            ...
            # TODO: This has not been modified for vectorized env
            if ignore_physics:
                self.robot[env_idx].set_joint_positions(joint_pos)
                og.sim.step()
                if stop_on_contact and detect_robot_collision_in_sim(self.robot[env_idx]):
                    return
            else:
                ...
                articulation_control_idx = [self.robot[0].trunk_control_idx]
                for arm in self.robot[0].arm_names:
                    articulation_control_idx.append(self.robot[0].arm_control_idx[arm])
                ...
                for j in range(m.MAX_STEPS_FOR_JOINT_MOTION):
                    ...
                    # current_joint_pos = self.robot[env_idx].get_joint_positions()
                    # joint_pos_diff = joint_pos - current_joint_pos
                    # base_joint_diff = joint_pos_diff[self.robot[env_idx].base_control_idx]
                    ...
                    # collision_detected = detect_robot_collision_in_sim(self.robot[env_idx])
    ...
    def _move_hand_direct_joint(self, joint_pos, stop_on_contact=False, ignore_failure=False):
        ...
        for i in range(m.MAX_STEPS_FOR_HAND_MOVE_JOINT):
            current_joint_pos = self.robot[env_idx].get_joint_positions()[self._manipulation_control_idx()]
            ...
            if (
                ...
                and th.max(th.abs(self.robot[env_idx].get_eef_position(self.arm) - prev_eef_pos)).item() < 0.0001
            ):
                # We're stuck!
                break
            ...
            prev_eef_pos = self.robot[env_idx].get_eef_position(self.arm)
            yield self._postprocess_action(action)
    ...
    def _move_hand_direct_ik(
        ...
        controller_config = self.robot[env_idx]._controller_config["arm_" + self.arm]
        assert (
        ...
        for i in range(m.MAX_STEPS_FOR_HAND_MOVE_IK):
            current_pose = self._world_pose_to_robot_pose(
                (self.robot[env_idx].get_eef_position(self.arm), self.robot[env_idx].get_eef_orientation(self.arm))
            )
            ...
            # if i > 0 and stop_if_stuck and detect_robot_collision_in_sim(self.robot[env_idx]):
    ...
    def _move_hand_linearly_cartesian(
        ...
        start_pos, start_orn = self.robot[env_idx].eef_links[self.arm].get_position_orientation()
        ...
        if controller_config["name"] == "InverseKinematicsController":
            ...
            for i, waypoint in enumerate(waypoints):
                ...
                current_pos, current_orn = self.robot[env_idx].eef_links[self.arm].get_position_orientation()
            ...
            joints = list(self.robot[env_idx].joints.values())

            for joint_pos in joint_space_data:
                # Check if the movement can be done roughly linearly.
                current_joint_positions = self.robot[env_idx].get_joint_positions()[self._manipulation_control_idx()]
    ...
    def _get_joint_position_with_fingers_at_limit(self, limit_type):
        ...
        """
        target_joint_positions = self.robot[env_idx].get_joint_positions()
        gripper_ctrl_idx = self.robot[env_idx].gripper_control_idx[self.arm]
        if limit_type == self.robot[env_idx]._grasping_direction:
            finger_joint_limits = self.robot[env_idx].joint_lower_limits[gripper_ctrl_idx]
        else:
            finger_joint_limits = self.robot[env_idx].joint_upper_limits[gripper_ctrl_idx]
    ...
    def _move_fingers_to_limit(self, limit_type):
        ...
        action = self.robot[env_idx].q_to_action(target_joint_positions)
        for _ in range(m.MAX_STEPS_FOR_GRASP_OR_RELEASE):
            yield self._postprocess_action(action)
            current_joint_positions = self.robot[env_idx].get_joint_positions()
    ...
    def _overwrite_head_action(self, action):
        ...
        if self._always_track_eef:
            target_obj_pose = (self.robot[env_idx].get_eef_position(self.arm), self.robot[env_idx].get_eef_orientation(self.arm))
        else:
            if self._tracking_object is None:
                return action

            if self._tracking_object == self.robot[env_idx]:
                target_obj_pose = (self.robot[env_idx].get_eef_position(self.arm), self.robot[env_idx].get_eef_orientation(self.arm))
            else:
                target_obj_pose = self._tracking_object.get_position_orientation()

        assert isinstance(self.robot[env_idx], Tiago), "Tracking object with camera is currently only supported for Tiago"

        head_q = self._get_head_goal_q(target_obj_pose)
        head_idx = self.robot[env_idx].controller_action_idx["camera"]

        config = self.robot[env_idx]._controller_config["camera"]
        ...
        if use_delta:
            cur_head_q = self.robot[env_idx].get_joint_positions()[self.robot[env_idx].camera_control_idx]
    ...
    def _get_head_goal_q(self, target_obj_pose):
        ...
        head1_joint = self.robot[env_idx].joints["head_1_joint"]
        head2_joint = self.robot[env_idx].joints["head_2_joint"]
        ...
        robot_pose = self.robot[env_idx].get_position_orientation()
        ...
        if head1_joint_limits[0] < theta < head1_joint_limits[1]:
            ...
            head2_pose = self.robot[env_idx].links["head_2_link"].get_position_orientation()
        ...
        # if not possible to look at object, return the reset (default) head joint positions
            ...
            default_head_pos = self.robot[env_idx].reset_joint_pos[self.robot[env_idx].controller_action_idx["camera"]]
    ...
    def _empty_action(self, follow_arm_targets=True):
        ...
        """
        actions = []
        for env_idx in range(len(self.env.envs)):
            action = th.zeros(self.robot[env_idx].action_dim)
            for name, controller in self.robot[env_idx]._controllers.items():
                ...
                if follow_arm_targets and name in self._arm_targets:
                    # TODO: Fix this to work with evctorized env
                    if isinstance(controller, InverseKinematicsController):
                        ...
                        current_pos, current_orn = self._world_pose_to_robot_pose(
                            (self.robot[env_idx].get_eef_position(arm), self.robot[env_idx].get_eef_orientation(arm))
                        )
                        delta_pos = target_pos - current_pos
                        ...
                        target_joint_pos = self._arm_targets[f"{name}_env_{env_idx}"]
                        current_joint_pos = self.robot[env_idx].get_joint_positions()[self._manipulation_control_idx()]
                    ...
                    partial_action = controller.compute_no_op_action(self.robot[env_idx].get_control_dict())
                action_idx = self.robot[env_idx].controller_action_idx[name]
                action[action_idx] = partial_action
            actions.append(action)
        return th.stack(actions)
    ...
    def _reset_robot(self):
        ...
        target_pos = dict()
        target_quat = dict()
        for arm in self.robot[env_idx].arm_names:
            reset_eef_pose = self._get_reset_eef_pose("world")[arm]
            target_pos[self.robot[env_idx].eef_link_names[arm]] = reset_eef_pose[0]
            target_quat[self.robot[env_idx].eef_link_names[arm]] = reset_eef_pose[1]
        q_traj = self._plan_joint_motion(
            target_pos=target_pos,
            target_quat=target_quat,
            embodiment_selection=CuRoboEmbodimentSelection.ARM,
    ...
    def _navigate_to_pose(self, pose_2ds, skip_obstacle_update=False):
        """
        ...
        target_positions = []
        target_quats = []
        for env_idx, pose_2d in enumerate(pose_2ds):
            pose_3d = self._get_robot_pose_from_2d_pose(pose_2d, env_idx=env_idx)
            # if self.debug_visual_marker is not None:
            #     self.debug_visual_marker.set_position_orientation(*pose_3d)
            target_pos = {self.robot[env_idx].base_footprint_link_name: pose_3d[0]}
            target_quat = {self.robot[env_idx].base_footprint_link_name: pose_3d[1]}
        ...
        yield from self._execute_motion_plan(q_trajs)

    # (TODO) add a function to draw curobo-generated plans.
    def _draw_plan(self, plan):
        SEARCHED = []
        trav_map = self.robot[env_idx].scene._trav_map
        for q in plan:
    ...
    def _navigate_to_obj(self, obj_vec, eef_pose=None, skip_obstacle_update=False):
        ...
        for env_idx in range(self.num_envs):            
            ...
            if pose is None:
                # raise ActionPrimitiveError(
                #     ActionPrimitiveError.Reason.PLANNING_ERROR,
                #     f"Env {env_idx}: Could not find a valid pose near the object",
    ...
    def _navigate_to_pose_direct(self, pose_2d, low_precision=False):
        ...
        for _ in range(m.MAX_STEPS_FOR_WAYPOINT_NAVIGATION):
            ...
            diff_pos = end_pose[0] - self.robot[env_idx].get_position_orientation()[0]
            intermediate_pose = (
                end_pose[0],
            ...
            if abs(diff_yaw) > m.DEFAULT_ANGLE_THRESHOLD:
                ...
                action = self._empty_action()
                if isinstance(self.robot[env_idx].controllers["base"], HolonomicBaseJointController):
                    assert (
                        self.robot[env_idx].controllers["base"].motor_type == "velocity"
                    ), "Holonomic base controller must be in velocity mode"
                    direction_vec = (
                        body_target_pose[0][:2] / th.norm(body_target_pose[0][:2]) * m.KP_LIN_VEL[type(self.robot[env_idx])]
                    )
                    base_action = th.tensor([direction_vec[0], direction_vec[1], 0.0], dtype=th.float32)
                    action[self.robot[env_idx].controller_action_idx["base"]] = base_action
                elif isinstance(self.robot[env_idx].controllers["base"], DifferentialDriveController):
                    base_action = th.tensor([m.KP_LIN_VEL[type(self.robot[env_idx])], 0.0], dtype=th.float32)
                    action[self.robot[env_idx].controller_action_idx["base"]] = base_action
                else:
                    raise ValueError(f"Unsupported base controller: {type(self.robot[env_idx].controllers['base'])}")
    ...
    def _rotate_in_place(self, end_pose, angle_threshold=m.DEFAULT_ANGLE_THRESHOLD):
        ...
        for _ in range(m.MAX_STEPS_FOR_WAYPOINT_NAVIGATION):
            ...
            ang_vel = m.KP_ANGLE_VEL[type(self.robot[env_idx])] * direction

            base_action = action[self.robot[env_idx].controller_action_idx["base"]]
    ...
    def _sample_pose_near_object(
        ...
        avg_arm_workspace_range = th.mean(self.robot[env_idx].arm_workspace_range[self.arm])

        if eef_pose is None:
            eef_pose, _ = self._sample_grasp_pose(obj, env_idx=env_idx)
        ...
        # use local target_pose as SegmenationMap.get_room_instance_by_point() expects a local point (in scene frame and not world frame)
        target_point_for_room = obj.scene.convert_world_pose_to_scene_relative(target_pose[self.arm][0], target_pose[self.arm][1])[0][:2]

        obj_rooms = (
            obj.in_rooms
            if obj.in_rooms
            else [self.robot[env_idx].scene._seg_map.get_room_instance_by_point(target_point_for_room)]
        )
        ...
        while attempt < sampling_attempts:
            ...
            for _ in range(self._curobo_batch_size):
                ...
                for _ in range(m.MAX_ATTEMPTS_FOR_SAMPLING_POSE_FOR_CORRECT_ROOM):
                    ...
                    # Check room. Same thing: convert candidate_2d_pose[:2] to scene frame from world frame
                    candidate_position, candidate_quat = th.cat((candidate_2d_pose[:2], th.tensor([1.0]))), th.tensor(R.from_euler("z", candidate_2d_pose[2]).as_quat(), dtype=th.float32)
                    candidate_point = obj.scene.convert_world_pose_to_scene_relative(candidate_position, candidate_quat)[0][:2]
                    if self.robot[env_idx].scene._seg_map.get_room_instance_by_point(candidate_point) in obj_rooms:
    ...
    def _validate_poses(self, candidate_poses, env_idx=0, eef_pose=None, plan_with_open_gripper=False, skip_obstacle_update=False):
        """
        ...
        candidate_local_joint_positions, candidate_global_joint_positions = [], []
        if plan_with_open_gripper:
            current_joint_pos = self._get_joint_position_with_fingers_at_limit("upper")
        else:
            current_joint_pos = self.robot[env_idx].get_joint_positions()
        for pose2d_wrt_world in candidate_poses:
            joint_pos_local = current_joint_pos.clone()
            joint_pos_global = current_joint_pos.clone()
            # convert pose from world frame to scene frame
            # FIXME: Change 1.0 to 0.0 (z should be 0.0)? and see
            pos, quat = th.cat((pose2d_wrt_world[:2], th.tensor([1.0]))), th.tensor(R.from_euler("z", pose2d_wrt_world[2]).as_quat(), dtype=th.float32)
            pos_wrt_scene, quat_wrt_scene = self.robot[env_idx].scene.convert_world_pose_to_scene_relative(pos, quat)
            pose2d_wrt_scene = th.tensor([pos_wrt_scene[0], pos_wrt_scene[1], R.from_quat(quat_wrt_scene).as_euler("xyz")[2]], dtype=th.float32)

            # in OG, the robot bases (x,y,yaw) are represented as joints, hence there is a base joint in a local frame (scene frame) 
            # and in global frame (world frame)
            joint_pos_local[self.robot[env_idx].base_control_idx] = pose2d_wrt_scene
            candidate_local_joint_positions.append(joint_pos_local)

            joint_pos_global[self.robot[env_idx].base_control_idx] = pose2d_wrt_world
            candidate_global_joint_positions.append(joint_pos_global)

        candidate_local_joint_positions = th.stack(candidate_local_joint_positions)
        candidate_global_joint_positions = th.stack(candidate_global_joint_positions)

        # If an object is grasped, we need to pass it to the collision checker
        obj_in_hand = self._get_obj_in_hand(env_idx=env_idx)
        attached_obj = {self.robot[env_idx].eef_link_names[self.arm]: obj_in_hand.root_link} if obj_in_hand is not None else None

        # TODO: Currently validate poses is done sequentially for all envs and so is check_collision. Change to parallelized later
        ...
        invalid_results = self._motion_generator.check_collisions(
            candidate_global_joint_positions,
        ...
        for i in range(len(candidate_poses)):
            ...
            if eef_pose is not None:
                ...
                candidate_joint_position = candidate_local_joint_positions[i]
                if not self._target_in_reach_of_robot(
                    eef_pose, env_idx=env_idx, initial_joint_pos=candidate_joint_position, skip_obstacle_update=skip_obstacle_update
                ):
                    invalid_results[i] = True

        return ~invalid_results

    def _get_robot_pose_from_2d_pose(self, pose_2d, env_idx=0):
        """
        ...
        """
        if isinstance(self.robot[env_idx], HolonomicBaseRobot):
            base_joints = self.robot[env_idx].get_joint_positions()[self.robot[env_idx].base_idx]
    ...
    def _world_pose_to_robot_pose(self, pose):
        ...
        Returns:
            2-tuple:
                ...
                - 4-array: (x,y,z,w) Quaternion orientation in the world frame
        """
        body_pose = self.robot[env_idx].get_position_orientation()
    ...
    def _get_hand_pose_for_object_pose(self, desired_pose):
        ...
        hand_in_world = self.robot[env_idx].eef_links[self.arm].get_position_orientation()
    ...
    def _settle_robot(self):
        ...
        """
        for _ in range(50):
            empty_action = self.robot[env_idx].q_to_action(self.robot[env_idx].get_joint_positions())
            yield self._postprocess_action(empty_action)

        for _ in range(m.MAX_STEPS_FOR_SETTLING):
            if th.norm(self.robot[env_idx].get_linear_velocity()) < 0.01:
                break
            empty_action = self.robot[env_idx].q_to_action(self.robot[env_idx].get_joint_positions())
            yield self._postprocess_action(empty_action)
