...
def pose2mat(pose: Tuple[torch.Tensor, torch.Tensor]) -> torch.Tensor:
    ...
    homo_pose_mat = torch.eye(4, dtype=torch.float32, device=pos.device)
...
def axisangle2quat(vec: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    ...
    # Grab angle
    angle = torch.norm(vec, dim=-1, keepdim=True, dtype=torch.float32)

    # Create return array
    quat = torch.zeros(
        torch.prod(torch.tensor(input_shape, dtype=torch.int)), 4, device=vec.device, dtype=torch.float32
    )
    quat[:, 3] = 1.0
...
def pose_inv(pose_mat):
    ...
    pose_inv = torch.zeros((4, 4), dtype=pose_mat.dtype, device=pose_mat.device)
