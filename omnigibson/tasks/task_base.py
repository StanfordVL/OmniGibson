...
class BaseTask(GymObservable, Registerable, metaclass=ABCMeta):
    ...
    def _step_metrics(self, env, action, info=None):
        """
        ...
        Returns:
            2-tuple:
                - list[float]: metrics at the current timestep
                - dict: any information passed through this function or generated by this function
        """
        # Make sure info is a dict
        total_info = dict() if info is None else info
        ...
        for metric_name, metric_function in self._metric_functions.items():
            metric, metric_info = metric_function.step(self, env, action)
            total_metric += metric
            breakdown_dict[metric_name] = metric
            total_info[metric] = metric_info

        # Store breakdown dict
        total_info["metric_breakdown"] = breakdown_dict

        return total_metric, total_info
