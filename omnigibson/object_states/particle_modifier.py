...
def create_projection_visualization(
    ...
    source = lo.UsdGeom.Sphere.Define(og.sim.stage, lo.Sdf.Path(prim_path))
    ...
    lo.UsdGeom.Imageable(source.GetPrim()).MakeInvisible()
    ...
    instancer_prim = lo.get_prim_at_path(instancer_path)
    ...
    emitter_prim.GetProperty("inputs:scale").Set(lo.Gf.Vec3f(1.0, 1.0, 1.0))
    emitter_prim.GetProperty("inputs:directionRandom").Set(lo.Gf.Vec3f(*spread))
    ...
    return lo.get_prim_at_path(system_path), emitter_prim
    ...
    def _initialize(self):
        ...
        if self.method == ParticleModifyMethod.PROJECTION:
            ...
            if not pre_existing_mesh:
                ...
                mesh = lo.UsdGeom.__dict__[self._projection_mesh_params["type"]].Define(og.sim.stage, mesh_prim_path).GetPrim()
            ...
            projection_mesh_ids = lo.PhysicsSchemaTools.encodeSdfPath(self.projection_mesh.prim_path)
...
class ParticleApplier(ParticleModifier):
    ...
    def _initialize(self):
        ...
        if self.visualize:
            ...
            if lo.is_prim_path_valid(projection_path):
                self.projection_system = lo.get_prim_at_path(projection_path)
                self.projection_emitter = lo.get_prim_at_path(f"{projection_path}/emitter")
            else:
    ...
    def remove(self):
        ...
        if self.projection_system is not None:
            lo.delete_prim(self.projection_system.GetPrimPath().pathString)
