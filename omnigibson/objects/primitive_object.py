...
class PrimitiveObject(StatefulObject):
    ...
    def _load(self):
        ...
        lo.UsdPhysics.CollisionAPI.Apply(self._col_geom.GetPrim())
        lo.UsdPhysics.MeshCollisionAPI.Apply(self._col_geom.GetPrim())
        lo.PhysxSchema.PhysxCollisionAPI.Apply(self._col_geom.GetPrim())
    ...
    def radius(self, radius):
        ...
        for geom in self._vis_geom, self._col_geom:
            if geom is not None:
                ...
                geom.GetExtentAttr().Set(lo.Vt.Vec3fArray([lo.Gf.Vec3f(*(-self._extents / 2.0)), lo.Gf.Vec3f(*(self._extents / 2.0))]))
        ...
        for attr, vals in attr_pairs:
            ...
            # Set the value
            attr.Set(lo.Vt.Vec3fArray([lo.Gf.Vec3f(*v) for v in vals]))
