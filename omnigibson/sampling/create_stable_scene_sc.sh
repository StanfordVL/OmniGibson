...
ENROOT_MOUNT_HOME=no enroot start \
    ...
    ${MOUNT_KWARGS} \
    ${CONTAINER_NAME} \
    micromamba run -n omnigibson /bin/bash --login -c "cd / && git clone https://github.com/StanfordVL/bddl.git --branch develop --single-branch bddl-src && cd bddl-src && pip install -e . && cd / && mv omnigibson-src omnigibson-src-backup && git clone https://github.com/StanfordVL/OmniGibson.git --branch feat/sampling_2024 --single-branch omnigibson-src && cd /omnigibson-src && source /isaac-sim/setup_conda_env.sh && pip install gspread && python omnigibson/sampling/create_stable_scene.py"
