...
class OGRobotServer:
    def __init__(
        ...
        assert issubclass(robot_cls, ManipulationRobot), (
            f"Robot class {robot} is not a manipulation robot! Cannot use GELLO"
        )
        assert robot in SUPPORTED_ROBOTS, (
            f"Robot {robot} is not supported by GELLO! Supported robots: {SUPPORTED_ROBOTS}"
        )
    ...
    def _setup_teleop_support(self):
        ...
        self.camera_paths, self.viewports = utils.setup_cameras(
            self.robot, self.env.external_sensors, RESOLUTION
        )
    ...
    def _update_observations(self) -> Dict[str, th.tensor]:
        ...
        finger_impulses = (
            GripperRigidContactAPI.get_all_impulses(self.env.scene.idx)
        ...
        obs["base_contact"] = (
            any(
                len(link.contact_list()) > 0
                for link in self.robot.non_floor_touching_base_links
            )
            if INCLUDE_BASE_CONTACT_OBS
            else False
        )
        obs["trunk_contact"] = (
            any(len(link.contact_list()) > 0 for link in self.robot.trunk_links)
        ...
        for i, arm in enumerate(self.robot.arm_names):
            ...
            obs[f"arm_{arm}_contact"] = (
                any(len(link.contact_list()) > 0 for link in self.robot.arm_links[arm])
                if VIEWING_MODE != ViewingMode.VR and INCLUDE_ARM_CONTACT_OBS
                else False
            )
            obs[f"arm_{arm}_finger_max_contact"] = (
                th.max(
                    th.sum(
                        th.square(finger_impulses[:, 2 * i : 2 * (i + 1), :]), dim=-1
                    )
                ).item()
                if INCLUDE_FINGER_CONTACT_OBS
                else 0.0
            ...
            for obj in self.task_relevant_objects:
                ...
                if obj.fixed_base and obj.articulated:
                    for name, link in obj.links.items():
                        if "meta" not in name and link != obj.root_link:
    ...
    def reset(self, increment_instance=True):
        ...
        self._joint_cmd = {
            f"{arm}_arm": self._joint_state[self.robot.arm_control_idx[arm]]
            for arm in self.robot.arm_names
        }
        self._should_update_checkpoint = False
        self._grasp_action = {arm: 1 for arm in self.robot.arm_names}
        ...
        if self.instance_id is not None and increment_instance:
            ...
            for bddl_name, obj_state in tro_state.items():
                if "agent" in bddl_name:
                    ...
                    # for orientation
                    robot_pos = (
                        obj_state["joint_pos"][:3] + obj_state["root_link"]["pos"]
                    )
                    robot_quat = T.euler2quat(
                        th.tensor([0, 0, obj_state["joint_pos"][5]])
                    )
                    self.env.task.object_scope[bddl_name].set_position_orientation(
                        robot_pos, robot_quat
                    )
                else:
                    self.env.task.object_scope[bddl_name].load_state(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--recording-path", type=str, default=None, help="Path to save recordings"
    )
    parser.add_argument("--task", type=str, default=None, help="Task name")
    parser.add_argument(
        "--no-ghost", action="store_true", help="Disable ghost robot visualization"
    )
